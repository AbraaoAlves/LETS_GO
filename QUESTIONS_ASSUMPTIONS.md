...
> [ASSUMPTION]: Samples are immutable reference entities — named, typed, optionally linked by parent lineage ([A2](#a-language-and-core-concepts)), but not tracked for quantity or state changes. The problem statement describes samples as things experiments reference; it never names inventory management, depletion tracking, or state transitions. Under [A0](#a-language-and-core-concepts) (CSV ingestion of historical lab logs), the import records what samples were used, not how much was consumed or what physical state they were in at each step. This choice accepts two operational risks. First, a sample that has been consumed, destroyed, or disposed of in the physical world still appears linkable in the database — no status guard prevents a researcher from associating discarded bio-waste with a new experiment. This also applies after aliquoting ([A2](#a-language-and-core-concepts)): once a parent sample is fully fractionated into child vials, the parent remains a valid reference target even though it no longer physically exists in its original form. Second, `storage_location` is a point-in-time snapshot from the CSV import — any subsequent move (freezer rotation, bench work, disposal) is invisible, which is a chain-of-custody gap for regulated environments. Both risks are accepted because the scope is a historical log import, not a real-time operational system.
